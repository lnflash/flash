...
| W4 | **NEW-CASHOUT-V1-WALLET** | Cashout V1 source-wallet switch for opted-in JM users (USD → USDT with USD swap step) | High | **File.** Joint — Olaniran (Cashout V1 source-wallet selection logic: read account opt-in flag, switch source from legacy USD to ETH-USDT with a USDT→USD swap via IBEX before the JMD off-ramp) + Ben (account-flag plumbing / Cashout V1 GraphQL surface). Flagged to Dread: the pre-correction docs said "Cashout V1 unchanged" — this needs explicit product confirmation. |
...
| ongoing | **Cashout V1 contradiction flag** | Confirm NEW-CASHOUT-V1-WALLET scope with product | Pre-correction docs said "Cashout V1 unchanged". The opt-in moves opted-in JM users' Cash Wallet from legacy USD → ETH-USDT, so the Cashout V1 source-wallet must also flip (USDT → USD swap before JMD off-ramp). **Get explicit product confirmation before implementation.** |
...
- **JM Cashout V1 work** *broadly* — separate project (FLOWS §5). **Exception:** NEW-CASHOUT-V1-WALLET (source-wallet switch for opted-in users) is in this plan because it's a direct consequence of the Cash Wallet swap. Anything else about Cashout V1 UX/FX/limits stays on the Cashout V1 project.
