...
| **ENG-276** Deposit reconciliation Bridge↔IBEX | Backlog | Urgent | **Not in proposal — missed** | gap in proposal | Keep. Overlaps with the Bridge-fee-persistence acceptance and the backfill/replay-tooling acceptance (both folded into ENG-276). See §C flag #2 for de-dup. |
...
| **ENG-284** Idempotency in `createVirtualAccount` (HIGH-4) | In Progress | High | Partial overlap with ENG-349 | partial | Keep. ENG-284 is VA-creation only. ENG-349 is **withdrawal** idempotency — **still needed** as a separate ticket. See §C flag #3. |
...
| **ENG-286** Timeout + circuit breaker (MED-2) | Backlog | Medium | Split timeout (former NEW-5 outbound-timeout work, now folded into ENG-286 scope) from breaker | partial | **Re-scope in place** instead of splitting — ticket already lists both, just tag timeout as W3/launch-blocking and breaker as W7/post-launch. |
...
|---|---|---|---|
| **ENG-351/352** | GraphQL payload-shape mismatches | not filed | **File.** P0, launch-blocking. |
| **ENG-353** | GraphQL error-code differentiation | not filed | **File.** P0, launch-blocking. |
| **ENG-349** | Withdrawal idempotency key | not filed (ENG-284 is VA-only) | **File.** P0. Mirror ENG-284's approach but on `initiateWithdrawal`. |
| **ENG-350** | Refund on `transfer.failed` | not filed | **File.** P0, money risk. |
| _outbound-timeout work_ | Outbound Bridge HTTP timeout | bundled into ENG-286 | **Don't file.** Re-scope ENG-286. |
| **ENG-354** | KYC tier distinct error | not filed | **File.** P1, depends on ENG-353. |
| **ENG-355** | Min-withdrawal floor | not filed | **File.** P1, product-gated. |
| **ENG-358** | Add fee/fxRate columns to `bridgeWithdrawals` | not filed | **File.** P1, schema. |
| _Bridge-fee persistence_ | Persist Bridge `fee` from `GET /transfers/:id` | overlaps ENG-276 | **Don't file separately.** Fold into ENG-276 acceptance. |
| **ENG-359** | Move Bridge API key to vault | not filed | **File.** P2, security. |
| **ENG-360** | Webhook key rotation policy | not filed | **File.** P2, security. |
| _backfill/replay tooling_ | Backfill / replay tooling | overlaps ENG-276 | **Don't file separately.** Fold into ENG-276. |
| **ENG-363** | One-VA-per-account schema constraint | overlaps ENG-284 | **Fold into ENG-284** as acceptance. |
...
| **3** | **Withdrawal idempotency really is missing.** ENG-284 covers VA creation only. `BridgeClient.createTransfer` supports `Idempotency-Key` that the service never passes. | **File ENG-349 as proposed.** |
...
| **Re-scope description only** | 6 | ENG-272, ENG-273 (split), ENG-274, ENG-276 (absorbs Bridge-fee persistence + backfill/replay tooling), ENG-285, ENG-286 |
| **File new** | **10** | ENG-351/352, ENG-353, ENG-349, ENG-350, ENG-354, ENG-355, ENG-358, ENG-359, ENG-360, + "Implement 2% developer_fee_percent" |
| **Retract from proposal** | 4 | outbound-timeout (→ ENG-286), Bridge-fee persistence (→ ENG-276), backfill/replay tooling (→ ENG-276), one-VA-per-account schema constraint (→ sub-issue ENG-363 of ENG-284) |
