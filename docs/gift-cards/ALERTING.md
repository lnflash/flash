...
| `order-paid` | success | `purchaseGiftCard`, `settleOrderFromVendor`, reconcile `settleAsPaid` | Payment settled; awaiting the card. `statusHistory` reason is `payment-settled`, `vendor-reported-fulfilled`, `vendor-reported-payment` (a vendor `paidPendingFulfillment` vouch on an INVOICE_ISSUED/EXPIRED row, or a ref-less PAYMENT_PENDING one whose IBEX re-read could never answer — `paidPendingFulfillment` vouch, or the vendor's `refunded`, which lands PAID then REFUND_REQUIRED), `payment-settled-on-reconcile` or `payment-settled-after-expiry` (a late Success on an EXPIRED row) | none |
...
| `refund-required` | failed | `settleOrderFromVendor`, reconcile `processPaid` | **Money left Flash and no card came.** `error` is `vendor-failed`, `vendor-refunded`, or `fulfillment-timeout` (24 h in PAID and the final vendor poll answered `awaitingPayment`: the vendor never saw the payment; `meta.lastVendorPoll` is `awaiting-payment`. A vendor error, or a vendor still-in-progress answer, does not escalate) | **Page.** RUNBOOK (a). Every occurrence is a customer owed money |
| `fulfillment-stalled` | pending | reconcile `processPaid` | 24 h in PAID and the vendor still reports the order in progress (`meta.vendorStatus` `paidPendingFulfillment`: `held`, `senttofulfillment`, an unknown status, `fulfilled` without claim data). **Not** a refund: the card may still ship. Order stays PAID and keeps polling; paged once per order per process | **Page.** RUNBOOK (b) step 3: ask TBC about the order. It resolves itself to `order-fulfilled` or, if the vendor later answers `awaitingPayment`/`failed`/`refunded`, to `refund-required` |
...
| `would-reject` | pending | `authorizeGiftCardPurchase` (log-only) | A cap or velocity check would have refused; `meta.reason`, `meta.level`, `meta.productId`, `error`. Never an eligibility reason (`level-not-eligible`, `account-too-new`, `open-loop-not-allowed`): those refuse in every mode | Review weekly before flipping to enforce (RUNBOOK f). `reason: limits-unavailable` must be zero |
| `rejected` | failed | `authorizeGiftCardPurchase` | A check refused: any eligibility reason in any mode, or a cap/velocity/`limits-unavailable` reason in enforce | Warn on rate; investigate `limits-unavailable` immediately |
