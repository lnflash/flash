...
describe("Offers", () => {
  it("successfully makes and persists an offer using default config", async () => {
    ...
    expect(offer.details.ibexTransfer.amount).toEqual(send.amount)
    expect(offer.details.flashFee.amount).toEqual(2n)
    expect(offer.details.flashFee.currency).toEqual("USD")
    expect(offer.details.usdLiability.amount).toEqual(99n)
    expect(offer.details.usdLiability.currency).toEqual("USD")
    expect(offer.details.jmdLiability.amount).toEqual(157n)
    expect(offer.details.jmdLiability.currency).toEqual("JMD")
  })
})