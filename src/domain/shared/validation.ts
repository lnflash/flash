...
export const isValidated = <T>(
  value: T | Validated<T> | ValidationError[],
): value is Validated<T> => {
  return (
    !Array.isArray(value) &&
    typeof value === "object" &&
    value !== null &&
    validatedSymbol in value
  )
}
...
const checkWalletCurrency =
  (currency: WalletCurrency) => async (o: { wallet: Wallet }) => {
