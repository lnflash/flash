...
const replayEvent = async (event: {
    ...
    if (!routeKey) {
        baseLogger.warn({ eventType: event.event_type }, "Skipping unsupported event type for replay")
...
const main = async () => {
    ...
    let fetched = 0, success = 0, failed = 0, skipped = 0
    baseLogger.info({
        start: args.start,
        end: args.end,
    ...
    for await (const event of listAllEvents({ start_date: args.start, end_date: args.end, event_type: bridgeFilter })) {
        fetched++

        const { status, body } = await replayEvent(event)

        if (status >= 200 && status < 300) {
            success++
            baseLogger.info({ eventId: event.id, eventType: event.event_type, status, response: body }, "Successfully replayed event")
        } else if (status === 0) {
            skipped++
            baseLogger.info({ eventId: event.id, eventType: event.event_type, status, response: body }, "Skipped event")
        } else {
            failed++
            baseLogger.error({ eventId: event.id, eventType: event.event_type, status, response: body }, "Failed to replay event")
        }

        await new Promise(resolve => setTimeout(resolve, 100)) // small delay to avoid overwhelming the webhook server
    }

    baseLogger.info({ fetched, success, failed, skipped }, "Completed Bridge webhook replay")
}


setupMongoConnection().then(async (mongoose) => {
    await main()
