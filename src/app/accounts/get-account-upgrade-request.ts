...
export const getAccountUpgradeRequests = async (
  ...
  const upgradeRequests = await Promise.all(
    ids.slice(0, count || ids.length).map((_) => ErpNext.getAccountUpgradeRequestById(_)),
  )
  return findErrors(upgradeRequests)
}

const findErrors = (
  erpnextResponses: (AccountUpgradeRequest | UpgradeRequestQueryError)[],
): AccountUpgradeRequest[] | UpgradeRequestQueryError => {
  return (
    erpnextResponses.find(
      (r): r is UpgradeRequestQueryError => r instanceof UpgradeRequestQueryError,
