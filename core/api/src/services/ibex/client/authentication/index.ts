...
const storeTokens = async (signInResp: SignInResponse200): Promise<void> => {
  ...
  if (!accessToken)
    return Promise.reject(
      new IbexClientError("No access token found in Ibex response body"),
    )
  ...
  if (!refreshToken)
    return Promise.reject(
      new IbexClientError("No refresh token found in Ibex response body"),
    )
  const rtResp = await Redis.setRefreshToken(refreshToken, refreshTokenExpiresAt)

  if (atResp instanceof CacheServiceError)
    log.warn(`IBEX: Failed to write accessToken to redis cache: ${atResp.message}`)
  if (rtResp instanceof CacheServiceError)
    log.warn(`IBEX: Failed to write refreshToken to redis cache: ${rtResp.message}`)
}
...
const refreshAccessToken = async (): Promise<void | IbexAuthenticationError> => {
  ...
  if (tokenOrErr instanceof CacheServiceError)
    return new IbexAuthenticationError(tokenOrErr)
  try {
    const resp = (await IbexSDK.refreshAccessToken({ refreshToken: tokenOrErr })).data
    if (!resp.accessToken)
      return new IbexAuthenticationError("Did not receive access token")
...
export const withAuth = async <S, T>(
  apiCall: () => Promise<FetchResponse<S, T>>,
): Promise<T | IbexAuthenticationError> => {
  ...
  } else if (atResp instanceof CacheServiceError)
    return new IbexAuthenticationError(atResp)
